...
export function useRoomSettings() {
  const { user } = useAuth();
  const [state, setState] = useState<RoomSettingsState>({ maxGuests: {}, displayNames: {}, ghlCalendarIds: {} });
  ...
  const fetchSettings = useCallback(async () => {
    ...
    const { data } = await supabase
      .from('room_settings')
      .select('room_name, max_guests, display_name, ghl_calendar_id')
      .eq('user_id', user.id);
    ...
    setState({ maxGuests, displayNames, ghlCalendarIds });
    setLoading(false);
  }, [user]);

  useEffect(() => { fetchSettings(); }, [fetchSettings]);

  const updateRoomSettings = useCallback(async (roomName: string, maxGuests: number, displayName?: string, ghlCalendarId?: string) => {
    if (!user) return;
    const { error } = await supabase
      .from('room_settings')
      .upsert(
        { user_id: user.id, room_name: roomName, max_guests: maxGuests, display_name: displayName || null, ghl_calendar_id: ghlCalendarId || null } as any,
  ...
  return { settings, displayNames: state.displayNames, ghlCalendarIds: state.ghlCalendarIds, loading, updateRoomSettings, updateMaxGuests: updateRoomSettings, getMaxGuests, getDisplayName };
}
